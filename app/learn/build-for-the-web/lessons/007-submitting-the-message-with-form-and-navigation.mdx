...
import profileCardServerRejectionInvisible from '../images/008-moving-validation-to-the-server-and-handling-rejections/screenshots/profile-card-server-rejection-invisible.jpg?url'
import profileCardErrorWithMessageStillPresent from '../images/008-moving-validation-to-the-server-and-handling-rejections/screenshots/profile-card-error-with-message-still-present.jpg?url'
import profileCardSendEnabledDespiteInvalidInput from '../images/008-moving-validation-to-the-server-and-handling-rejections/screenshots/profile-card-send-enabled-despite-invalid.jpg?url'
import profileCardErrorClearedAfterTyping from '../images/008-moving-validation-to-the-server-and-handling-rejections/screenshots/profile-card-error-cleared-after-typing.jpg?url'
...
- This is the moment the app stops being just JavaScript and becomes a web
  application.
...
### 1️⃣ Profile Card While Submitting
...
### 2️⃣ Profile Card After Sending
...
### 3️⃣ Complete Code for this Lesson 7
...
## Step 2: Replace Fake Sending with Real Submission 🧩
...
> - ⚠️ We are **not** changing the Actions section
>   (`<div className='mt-6 flex gap-3'>`).
> - We are only changing the wrapper around the **composer UI** — the part
>   that appears _after_ you click **Message**.

> ℹ️ We intentionally do not specify an action attribute on `<Form>`.
...
- We have only changed who is responsible for sending:
  > From JavaScript → to the browser.
- That is the foundation for the next step.
- At this point, the browser knows _how_ to send the data —  
  but nothing on the server knows what to do with it yet.
- That's what we add next.

> Now the browser can send data — but nothing is listening yet.

<hr className='my-6' />

## Step 3: Add the Server Action ⚙️
...
- We are not yet using the server's response in the UI — that is the next step.

<hr className='my-6' />

## Step 4: Observe Navigation State 🔄
...
- The UI is no longer blind — it is now aware of what the system is doing.
...
## Step 5: React to Server Feedback 🎯
...
- The system is now end-to-end:
  > User → Browser → Router → Server → Router → UI
...
- Forms are not UI primitives — they are protocol primitives.
- Submission is not a function call — it is navigation.
- Servers do not push into your UI — your UI _reacts_ to them.
- You do not control the flow — you participate in it.

<hr className='my-8' />

## What's Next → 🚀

- In the next lesson, we will introduce **rejection**:
  - What happens when the server says "no"?
  - What happens when submitted data is not accepted?
  - How does the UI react when the system rejects input?
- This is where the system becomes authoritative instead of permissive.
