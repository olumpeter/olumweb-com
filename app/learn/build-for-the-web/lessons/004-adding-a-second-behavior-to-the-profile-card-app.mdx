...
- In this lesson, we will **add one more** — and see how multiple pieces of state work together.
...
- Behaviorally, it will feel more interactive.

### 1️⃣ Before Clicking the Message Button
...
### 2️⃣ After Clicking the Message Button Once
...
### 3️⃣ After Clicking Cancel (Before Sending the First Message)
...
- This uses the same code as above — only the state changed back to `false`.
- No new code needed — and this itself teaches that state drives UI.

### 4️⃣ After Sending One Message
...
- We will not change its layout or styling — only its behavior.
...
## Step 3: Introduce Composing State ⚙️
...
- Each piece of state controls one concern:
  - `isFollowing` → relationship state
  - `isComposing` → UI state

<hr className='my-8' />

## Step 4: Connect the Message Button to State 🖱️
...
## Step 5: Render the Composer Conditionally ✍️
...
{/*
	...
	We are not showing or hiding it — we are describing when it should exist.
*/}
{isComposing && (
	<div className='mt-4 space-y-2'>
		<textarea
...
- Send doesn't do anything yet — we'll handle that next.

### After Clicking the Message Button Once 👀
...
## Step 7: Show Feedback to the User 👀
...
- Each interaction becomes:
  > Event → State change → Re-render → New UI
...
- Right now, your mental model is clean:
  ...
  > The UI is derived from state — not pushed into being.
